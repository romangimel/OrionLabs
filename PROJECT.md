...
The product owner reports that Production uses the one available Vercel Firewall custom rate-limit rule for `/api/generate-report`, with an intended threshold of 5 requests per 60 seconds per IP. This dashboard-only rule is not proven by the repository and still requires external release verification. The browser handles a plain upstream HTTP 429 as the same broad temporary-capacity state without assuming its source.
