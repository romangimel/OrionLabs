...
The report server accepts only the strict `ReportGenerationInput` shape, limits request size, and rejects unexpected fields. Gemini 3.7 Flash uses Low thinking and a 90-second provider timeout within a 120-second Vercel Function limit. This is reliability headroom for unusually slow valid generations, not the expected UX target. It receives frozen system/report instructions plus the approved runtime data. Structured output is generated from the same Zod schema used to validate the result, and application-controlled identity and focus data must still match the request.
...
The Gemini SDK's implicit retry loop is disabled. OrionLabs performs one initial request and at most one retry for transient provider failures or malformed output, but only when another full provider attempt can fit within a 110-second internal budget. Ten seconds remain reserved for parsing, validation, error handling, serialization, and HTTP completion. Browser errors never include provider internals, stack traces, API keys, or questionnaire free text.
